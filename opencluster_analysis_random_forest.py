...
for s in range(10):
    ...
    dfs = df1.sample(frac=0.8, random_state=s)
    p1 = dfs.pmra
    p2 = dfs.pmdec
    p3 = dfs.parallax
    p4 = dfs.ra
    ...
    XX = np.column_stack([dfs.designation,X])
    print('Final data length before gmm : ', len(dfs))
    ...
    #print('pmra:  ',kp[c,1][0], ' +- ' , np.sqrt(pcov[c][0][0][0]/len(dfs))) 
    #print('pmdec: ',kp[c,2][0], ' +- ' , np.sqrt(pcov[c][0][1][1]/len(dfs)))
    #print('plx:   ',kp[c,3][0], ' +- ' , np.sqrt(pcov[c][0][2][2]/len(dfs)))
    #print('ra:    ',kp[c,4][0], ' +- ' , np.sqrt(pcov[c][0][3][3]/len(dfs)))
    #print('dec:   ',kp[c,5][0], ' +- ' , np.sqrt(pcov[c][0][4][4]/len(dfs)))

    #SSE PARAMETER PLOTS
    sse =np.zeros(1)
    for i in range(gnum):
        c = np.array([i])
        ssec = pcov[c][0][0][0] + pcov[c][0][1][1] + pcov[c][0][2][2] + pcov[c][0][3][3] + pcov[c][0][4][4] 
        sse = np.vstack((sse,ssec))
    sse = sse[1:]
    print(sse)
    c = np.where(sse==np.min(sse))[0] + 1
    c = np.array([c])
    plt.errorbar(s,kp[c,1][0], np.sqrt(pcov[c][0][0][0]), marker='s', ms=2, mew=2,label='pmra'+str(s))
plt.legend()
plt.show()
